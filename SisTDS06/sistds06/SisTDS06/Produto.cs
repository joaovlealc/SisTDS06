...
namespace SisTDS06
{
    class Produto
    {
        ...
        public void Inserir(string nome, int quantidade, decimal valor)
        {
            SqlConnection con = ClassConecta.ObterConexao();
            SqlCommand cmd = con.CreateCommand();
            cmd.CommandText = "INSERT INTO produto(nome,quantidade,valor) VALUES ('"+nome+"',convert(int,'"+quantidade+"'),convert(decimal,'"+valor+"'))";
            cmd.CommandType = CommandType.Text;
            cmd.ExecuteNonQuery();
        ...
        public void Atualizar(int id, string nome, int quantidade, decimal valor)
        {
            SqlConnection con = ClassConecta.ObterConexao();
            SqlCommand cmd = con.CreateCommand();
            cmd.CommandText = "UPDATE produto SET nome='" + nome + "',quantidade=convert(int,'" + quantidade + "'),valor=convert(decimal,'" + valor + "') WHERE Id = '" + id + "'";
            cmd.CommandType = CommandType.Text;
            cmd.ExecuteNonQuery();
            ClassConecta.FecharConexao();
        }
