...
namespace SisTDS06
{
    class Usuario
    {
        ...
        public void Inserir(string nome, string login, string senha, string celular, DateTime dt_nascimento, DateTime dt_admissao, string cep, string endereco, string cidade, string bairro, string funcao, string email)
        {
            SqlConnection con = ClassConecta.ObterConexao();
            SqlCommand cmd = con.CreateCommand();
            cmd.CommandText = "INSERT INTO usuario(nome,login,senha,celular,dt_nascimento,dt_admissao,cep,endereco,cidade,bairro,funcao,email) VALUES ('"+nome+"','"+login+"','"+senha+"','"+celular+ "',Convert(DateTime,'"+dt_nascimento+"',111),Convert(DateTime,'"+dt_admissao+"',111),'" + cep+"','"+endereco+"','"+cidade+"','"+bairro+"','"+funcao+"','"+email+"')";
            cmd.CommandType = CommandType.Text;
            cmd.ExecuteNonQuery();
        ...
        public void Atualizar(int id, string nome, string login, string senha, string celular, DateTime dt_nascimento, DateTime dt_admissao, string cep, string endereco, string cidade, string bairro, string funcao, string email)
        {
            SqlConnection con = ClassConecta.ObterConexao();
            SqlCommand cmd = con.CreateCommand();
            cmd.CommandText = "UPDATE usuario SET nome='" + nome + "',login='" + login + "',senha='" + senha + "',celular='" + celular + "',dt_nascimento=Convert(DateTime,'" + dt_nascimento + "',111),dt_admissao=Convert(DateTime,'" + dt_admissao + "',111),cep='" + cep + "',endereco='" + endereco + "',cidade='" + cidade + "',bairro='" + bairro + "',funcao='" + funcao + "',email='" + email + "' WHERE Id = '"+id+"'";
            cmd.CommandType = CommandType.Text;
            cmd.ExecuteNonQuery();
            ClassConecta.FecharConexao();
        }
